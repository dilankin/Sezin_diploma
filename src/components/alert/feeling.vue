          ...
          <!-- <div v-for="i in feelings.length" :key="i" style="position: relative;">
            ...
            <div v-if="i - 1 === selectedItemIndex" class="save-button">
              <svg
                ...
                v-if="!localFeel.some(item => item.id === feelings[i-1].id) && getAuth"
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
              ...
              <p v-if="!localFeel.some(item => item.id === feelings[i-1].id) && getAuth"  @click="saveFeeling">{{ $t("save") }}</p>
            </div>
          </div> -->
...
export default {
  ...
  mounted() {
    ...
    this.localFeel = JSON.parse(localStorage.getItem('myFeelings')) || [];
  },
  computed: {
    ...mapGetters(["getAuth"])
  },
  methods: {
    ...
    saveFeeling() {
      ...
      myFeelings.push(this.feelings[this.selectedItemIndex]);
      this.localFeel = myFeelings
