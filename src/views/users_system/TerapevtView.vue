      ...
      <div class="admin">
        <div class="admin__navs">
          <div
            ...
            :key="item"
            @click="$router.push('/account/' + item.slug)"
          >
            {{ $t(item.title) }}
          </div>
        </div>
        <div class="admin__content">
          ...
          <div class="admin__content-statistics" v-if="currentroute === 'chat'">
            <h2>{{ $t("chats") }}</h2>
            <vue-advanced-chat
              :current-user-id="getUser.fullName"
          ...
          <div
            class="admin__content-vebinars"
            v-if="currentroute === 'vebinars'"
          >
...
export default {
  ...
  mounted() {
    ...
    socket.on("rooms", (data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        const filteredUsers = data[i].users.filter(
          (user) => user.email !== this.getUser.email
        );
        const fullNames = filteredUsers.map((user) => user.fullName);
        rooms.push({
          roomId: data[i].id, // Изменяем поле "id" на "roomId"
          roomName: fullNames[0], // Добавляем поле "roomName" с соответствующим значением
          createAt: data[i].createAt,
          updatedAt: data[i].updatedAt,
          combination: data[i].combination,
          users: data[i].users.map((user) => {
            return {
  ...
  methods: {
    ...
    async fetchMess(room, options) {
      clearInterval(this.sesss)
      console.log(room);
      this.sesss = setInterval(async () => {
         await this.$axios
        .get(`chat/${room.detail[0].room.roomId}`)
        .then((res) => {
          if (res.data.length) {
            const rooms = [];
            let dddata = res.data.sort((a, b) => a.id - b.id)
            for (let i = 0; i < dddata.length; i++) {
              rooms.push({
                _id: dddata[i].id,
                content: dddata[i].text,
                createAt: dddata[i].createAt,
                updatedAt: dddata[i].updatedAt,
                timestamp: dddata[i].id,
                indexId: dddata[i].id,
                date: dddata[i].createAt.slice(0, 10),
                senderId: dddata[i].user.fullName,
                user: {
                  _id: dddata[i].user.id,
                  email: dddata[i].user.email,
                  fullName: dddata[i].user.fullName,
                  score: dddata[i].user.score,
                  isSubscriber: res.data[i].user.isSubscriber,
                },
              });
            }
            this.messages = rooms;
            ...
            this.messages = [
              {
                ...
                createAt: "2023-05-29T16:42:41.494Z",
                updatedAt: "2023-05-29T16:42:41.494Z",
                senderId: room.detail[0].room.users[0].id,
                user: {
                  _id: room.detail[0].room.users[1].id,
                  email: room.detail[0].room.users[1].email,
                  fullName: room.detail[0].room.users[1].fullName,
                  score: 0,
                  isSubscriber: false,
                },
              },
    ...
    sendMessage(roomId) {
      ...
      this.messages.push({
        content: roomId.detail[0].content,
      ...
      this.$axios
        ...
        .then((res) => {
          this.articles = res.data.filter(
            (item) => item.user.email === this.getUser.email
          );
        })
        .catch((err) => {
          console.log(err);
...
.admin {
  ...
  &__navs {
    ...
    &-button {
      ...
      text-align: center;
      font-size: min(
        max(13px, calc(0.8125rem + ((1vw - 4.2px) * 0.4902))),
        18px
      );
    ...
    &-vebinars {
      ...
      &-title {
        ...
        p {
          ...
          font-weight: 300;
          font-size: min(
            max(19px, calc(1.1875rem + ((1vw - 4.2px) * 1.6667))),
            36px
          );
        }
        div {
